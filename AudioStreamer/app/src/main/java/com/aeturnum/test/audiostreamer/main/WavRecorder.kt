...
class WavRecorder(val context: Context) {
    ...
    fun startRecording(_filename: String? = null, internalStorage: Boolean = false) {
        val filename = _filename ?: "mclef_recording_${System.currentTimeMillis()}.wav"
        //val filename = _filename ?: "mclef_recording_${System.currentTimeMillis()}.wav"

//        path = if (internalStorage) context.filesDir?.path + "/$filename"
//        else context.externalCacheDir?.path + "/$filename"
        path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).absolutePath +  "/$filename"
        Helper.setLastRecFilePath(path)
    ...
    fun stopRecording():String {
        ...
        return  Helper.getLastRecFilePath()
    }
